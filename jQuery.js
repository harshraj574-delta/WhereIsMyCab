		...
		function OnSuccess3(data, status) {
			...
			$.each(JSON.parse(data), function (key, val) {
										...
										//setTimeout(function () {
											...
											*/
											...
											var myinterval1=setInterval(function () {
												...
												if ((!val.actVehicleEndTime) ||  clientid=="12" || clientid=="5" || clientid=="10"  ) {
				...
				setTimeout(function () {
					...
					setInterval(function () {
						curStatus = myTrackingStatus;
						curLastStatus = trip_status;
						if (curStatus != lastStatus || curLastStatus != lastTripStatus) {
							//	location.reload();
						}
					}, 2000);
				}, 5000);
...
console.log("1");
				...
				*/
				...
				function createinfowindow(marker, point, deviceid, heading) {
					function distfunction() {


						infowindow.setContent("<div class='info-window info-content'> " + distancefrom + " Km away </div>");
					...
					setTimeout(function () {
						console.log("3");
						//alert(myTrackingStatus)
						if (trip_status == "Completed" || myTrackingStatus == "Boarded"|| myTrackingStatus == "No-Show") {
							infowindow.close();
						} else {
					...
					setInterval(distfunction, 1000)

					google.maps.event.addListener(marker, 'click', distfunction);
